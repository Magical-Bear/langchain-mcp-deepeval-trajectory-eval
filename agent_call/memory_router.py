...
核心设计：LangGraph server 本身是 memory store，客户端只负责路由 + 注入。
"""
import os
import uuid
...
class MemoryRouter:
    def __init__(self, langgraph_base_url: str, assistant_id: str):
        ...
        self._threads_cache: Optional[list[dict]] = None
    ...
    def _extract_turns(self, messages: list[dict], n: int = 3) -> list[ConversationTurn]:
        ...
        pairs = list(zip(reversed(human_msgs), reversed(ai_msgs)))
        turns = []
    ...
    def _format_full_turns_for_classifier(
...
def get_memory_router(
    ...
    if _memory_router is None:
        _memory_router = MemoryRouter(
            ...
            or os.getenv("LANGGRAPH_ASSISTANT_ID", "my_mcp_agent"),
        )
